...
class IEEEDataset(Dataset):
    ...
    def __init__(self, data_path=None, sequence_length=100, prediction_horizon=1,
                 mode='single_step', train_ratio=0.7, target_column='Utot (V)'):
        ...
        if data_path is None:
            raise ValueError("数据路径必须指定")

        # 加载数据
        self.data = pd.read_csv(data_path)

        # 删除指定的列
        columns_to_drop = ['Time (h)', 'U1 (V)', 'U2 (V)', 'U3 (V)', 'U4 (V)', 'U5 (V)']
        available_columns = [col for col in columns_to_drop if col in self.data.columns]
        self.data = self.data.drop(columns_to_drop, axis=1)

        print(f"已删除列: {available_columns}")
        print(f"剩余列: {list(self.data.columns)}")
...
class IEEEDataLoader:
    def __init__(self, batch_size=32, shuffle=True, **dataset_args):
        self.dataset = IEEEDataset(**dataset_args)
...
def plot_results(train_losses, test_losses, predictions, targets, metrics, fc_name):
    ...
    axes[1, 0].set_ylabel('Total Voltage (V)')
...
def train_and_evaluate_model(file_path, fc_name):
    """训练和评估单个模型"""
    print(f"\n=== {fc_name} Voltage Prediction Demo ===")
    ...
    plot_results(trainer.train_losses, trainer.test_losses, predictions, targets, metrics, fc_name)
...
def main():
    """主函数：完整的训练和测试流程"""
    print("=== IEEE PEMFC Voltage Prediction Demo ===")
    ...
    if not os.path.exists(fc1_path):
        print(f"错误: 文件不存在 - {fc1_path}")
        return
    if not os.path.exists(fc2_path):
        print(f"错误: 文件不存在 - {fc2_path}")
        return

    # 训练FC1模型
    print("\n" + "=" * 50)
    trainer_fc1, metrics_fc1 = train_and_evaluate_model(fc1_path, "FC1")

    # 训练FC2模型
    print("\n" + "=" * 50)
    trainer_fc2, metrics_fc2 = train_and_evaluate_model(fc2_path, "FC2")

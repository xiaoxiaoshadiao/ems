...
class Trainer:
    ...
    def train_epoch(self):
        ...
        for x, y in self.train_loader:
            ...
            self.opt.zero_grad()
            pred = self.model.forecast(x, None, None, None)  # 调用 TimeMixer 的 forecast
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for x, y in self.test_loader:
                x, y = x.to(self.device), y.to(self.device)
                pred = self.model.forecast(x, None, None, None)
                loss = self.loss_fn(pred.squeeze(-1), y)
                total += loss.item()
...
class Normalize(nn.Module):
    ...
    def _denormalize(self, x):
        ...
        if self.affine:
            x -= self.affine_bias
            x /= self.affine_weight + self.eps * self.eps
        x *= self.stdev
        if self.subtract_last:
            x += self.last
        else:
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.final_fc = nn.Linear(configs.enc_in, 1)  # enc_in=8，对应输入特征数
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        dec_out = self.final_fc(dec_out)  # 输入张量，输出 [B, pred_len, 1]
        return dec_out
...
class Configs:
    def __init__(self):
        ...
        self.e_layers = 1
        self.decomp_method = 'moving_avg'  # 或 'dft_decomp'
        self.down_sampling_method = 'avg'  # 可选 'max' / 'avg' / 'conv'
...
def main():
    ...
    configs = Configs()
    model = Model(configs)  # 这里直接用你已有的 TimeMixer Model 类
    ...
    plt.plot(range(sample_size), tars[:sample_size], label='True Voltage')
    plt.plot(range(sample_size), preds[:sample_size], label='Predicted Voltage')
    plt.legend();
    plt.grid(True, alpha=0.3);
    plt.show()
